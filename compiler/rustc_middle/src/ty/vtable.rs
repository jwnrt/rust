...
pub(super) fn vtable_allocation_provider<'tcx>(
    ...
    let addr_size = tcx.data_layout.address_size;
    let ptr_align = tcx.data_layout.pointer_align.abi;

    let vtable_len = u64::try_from(vtable_entries.len()).unwrap();
    let vtable_size = ptr_size * (vtable_len - 2) + addr_size * 2;
    ...
    for entry in vtable_entries.iter() {
        let (scalar, size) = match *entry {
            VtblEntry::MetadataDropInPlace => {
                if ty.needs_drop(tcx, ty::TypingEnv::fully_monomorphized()) {
                    let instance = ty::Instance::resolve_drop_in_place(tcx, ty);
                    let fn_alloc_id = tcx.reserve_and_set_fn_alloc(instance, CTFE_ALLOC_SALT);
                    let fn_ptr = Pointer::from(fn_alloc_id);
                    (Scalar::from_pointer(fn_ptr, &tcx), ptr_size)
                } else {
                    (Scalar::from_maybe_pointer(Pointer::null(), &tcx), ptr_size)
                }
            }
            VtblEntry::MetadataSize => (Scalar::from_uint(size, addr_size), addr_size),
            VtblEntry::MetadataAlign => (Scalar::from_uint(align, addr_size), addr_size),
            ...
            VtblEntry::TraitVPtr(trait_ref) => {
                ...
                (Scalar::from_pointer(vptr, &tcx), ptr_size)
            }
        };
        vtable
            .write_scalar(&tcx, alloc_range(pos, size), scalar)
            .expect("failed to build vtable representation");
        pos += size;
    }
